    ...
    window.unsubscribeLiveSnapshot = onSnapshot(q, (snapshot) => {
        ...
        if (grid) {
            ...
            snapshot.forEach((docSnap, index) => {
                ...
                const finalClassName = `live-st-card student-view-card ${meClass}`;
                const finalCSSText = `background:white; border-radius:15px; padding:20px; display:flex; flex-direction:column; align-items:center; opacity:${opacityVal}; transition:0.3s; width:100%; max-width: 320px; margin: 0 auto; border: ${borderStyle}; position: relative; overflow: visible !important;`;

                const finalInnerHTML = `
                    ${isMe ? '<div class="me-badge">أنت</div>' : ''}
                    ${countBadge}
                    <div style="cursor:pointer; width:70px; height:70px; border-radius:50%; background:#f8fafc; border:3.5px solid ${statusColor}; display:flex; align-items:center; justify-content:center; font-size:30px; color:#0284c7; margin-bottom:10px; z-index:2;">
                        <i class="fa-solid ${s.avatarClass || 'fa-user-graduate'}"></i>
                    </div>
                    <div style="text-align:center;">
                        <div class="st-name notranslate" translate="no" style="cursor:pointer; font-size:16px; font-weight:900; color:#1e293b; text-decoration:none; text-align: center; direction: auto;">
                            ${s.name}
                        </div>
                ...
                const dataSignature = JSON.stringify({
                    ...
                    sg: segCount, nm: s.name, av: s.avatarClass
                });
