...
import { i18n, t, changeLanguage } from './i18n.js?v=2';
...
const SessionManager = (() => {
    ...
    async function _recoverActiveSession(uid) {
        let userRegSnap = null;
        try {
            userRegSnap = await getDoc(doc(db, 'user_registrations', uid));
            const liveState = userRegSnap.data()?.liveState;
        ...
        try {
            let college = sessionStorage.getItem('student_college');
            if (!college) {
                const regData = userRegSnap?.exists() ? userRegSnap.data() : null;
                college = regData?.college || regData?.registrationInfo?.college || '';
                if (college) sessionStorage.setItem('student_college', college);
            }
    ...
    async function searchForSession() {
        ...
        try {
            let college = sessionStorage.getItem('student_college');
            if (!college) {
                const regSnap = await getDoc(doc(db, 'user_registrations', auth.currentUser?.uid || ''));
                college = regSnap.exists() && (regSnap.data().college || regSnap.data().registrationInfo?.college) || '';
                if (college) sessionStorage.setItem('student_college', college);
            }
            if (!college) {
                UI.showToast('⚠️ تعذر تحديد كليتك، سجل الدخول مرة أخرى', 4000, '#ef4444');
                return;
            }
            ...
            if (sessionData.startTime) {
                ...
                const codeDeadlineMs = doorOpenMs + 5_000;
    ...
    async function submitFeedback() {
        ...
        try {
            ...
            const [attSnap, studentSnap] = await Promise.all([
                getDoc(attRef),
                getDoc(doc(db, 'user_registrations', auth.currentUser.uid)),
            ]);
            if (!attSnap.exists()) throw new Error('بيانات الحضور غير موجودة');
            const room = attSnap.data();
            const studentData = studentSnap.exists() ? studentSnap.data() : {};
            const studentCollege = studentData.college || studentData.registrationInfo?.college || null;
            ...
            batch.set(doc(collection(db, 'feedback_reports')), {
                ...
                doctorCollege: room.college || null,
...
const SmartSearch = (() => {
    ...
    async function startSmartSearch() {
        ...
        try {
            let college = sessionStorage.getItem('student_college');
            if (!college) {
                const regSnap = await getDoc(doc(db, 'user_registrations', window.auth.currentUser?.uid || ''));
                college = regSnap.exists() && (regSnap.data().college || regSnap.data().registrationInfo?.college) || '';
                if (college) sessionStorage.setItem('student_college', college);
            }
            if (!college) {
                content.innerHTML = `<div style="color:#ef4444;text-align:center;padding:20px;">⚠️ تعذر تحديد كليتك، سجل الدخول مرة أخرى</div>`;
                return;
            }
